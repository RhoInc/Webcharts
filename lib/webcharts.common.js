...
function yScaleAxis(max_range, domain, type) {
  ...
  if (type === 'log') {
    ...
    y = _d3.d3.scale.ordinal();
  } else if (type === 'time') {
    y = _d3.d3.time.scale();
  } else {
    y = _d3.d3.scale.linear();
  }
  ...
  var yAxis = _d3.d3.svg.axis().scale(y).orient('left').ticks(tick_count).tickFormat(type === 'ordinal' ? null : type === 'time' ? _d3.d3.time.format(y_format) : _d3.d3.format(y_format)).tickValues(config.y.ticks ? config.y.ticks : null).innerTickSize(6).outerTickSize(3);
...
function xScaleAxis(max_range, domain, type) {
  ...
  if (type === 'log') {
    ...
    x = _d3.d3.scale.ordinal();
  } else if (type === 'time') {
    x = _d3.d3.time.scale();
  } else {
    x = _d3.d3.scale.linear();
  }
  ...
  var tick_count = Math.max(2, Math.min(max_range / 80, 8));
  var xAxis = _d3.d3.svg.axis().scale(x).orient(config.x.location).ticks(tick_count).tickFormat(type === 'ordinal' ? null : type === 'time' ? _d3.d3.time.format(format) : _d3.d3.format(format)).tickValues(config.x.ticks ? config.x.ticks : null).innerTickSize(6).outerTickSize(3);
...
function summarize(vals, operation) {
  ...
  var mathed = stat === 'count' ? vals.length : stat === 'percent' ? vals.length : _d3.d3[stat](nvals);
...
function transformData(raw, mark) {
  ...
  var raw_dom_x = mark.summarizeX === 'cumulative' ? [0, raw.length] : config.x.type === 'ordinal' ? _d3.d3.set(raw.map(function (m) {
    return m[config.x.column];
  })).values().filter(function (f) {
    return f;
  }) : mark.split && mark.arrange !== 'stacked' ? _d3.d3.extent(_d3.d3.merge(raw_nest.nested.map(function (m) {
    return m.values.map(function (p) {
      return p.values.raw.length;
    });
  }))) : mark.summarizeX === 'count' ? _d3.d3.extent(raw_nest.nested.map(function (m) {
    return m.values.raw.length;
  })) : _d3.d3.extent(raw.map(function (m) {
    return +m[config.x.column];
  }).filter(function (f) {
    return +f || +f === 0;
  }));

  var raw_dom_y = mark.summarizeY === 'cumulative' ? [0, raw.length] : config.y.type === 'ordinal' ? _d3.d3.set(raw.map(function (m) {
  ...
  }))) : mark.summarizeY === 'count' ? _d3.d3.extent(raw_nest.nested.map(function (m) {
    return m.values.raw.length;
  })) : _d3.d3.extent(raw.map(function (m) {
    return +m[config.y.column];
  }).filter(function (f) {
  ...
  function makeNest(entries, sublevel) {
    ...
    if (sublevel) {
      ...
      this_nest.sortKeys(function (a, b) {
        return config.x.type === 'time' ? _d3.d3.ascending(new Date(a), new Date(b)) : config.x_dom ? _d3.d3.ascending(config.x_dom.indexOf(a), config.x_dom.indexOf(b)) : sublevel === config.color_by && config.legend.order ? _d3.d3.ascending(config.legend.order.indexOf(a), config.legend.order.indexOf(b)) : config.x.type === 'ordinal' || config.y.type === 'ordinal' ? naturalSorter(a, b) : _d3.d3.ascending(+a, +b);
      });
    }
    this_nest.rollup(function (r) {
      ...
      obj.x_q25 = config.error_bars && config.y.type === 'ordinal' ? _d3.d3.quantile(x_vals, 0.25) : obj.x;
      obj.x_q75 = config.error_bars && config.y.type === 'ordinal' ? _d3.d3.quantile(x_vals, 0.75) : obj.x;
      obj.y_q25 = config.error_bars ? _d3.d3.quantile(y_vals, 0.25) : obj.y;
      obj.y_q75 = config.error_bars ? _d3.d3.quantile(y_vals, 0.75) : obj.y;
      ...
      if (mark.summarizeY === 'cumulative') {
        ...
        var cumul = config.x.type === 'time' ? interm.length : _d3.d3.sum(interm.map(function (m) {
    ...
    var dom_x = _d3.d3.extent(_d3.d3.merge(dom_xs));
    var dom_y = _d3.d3.extent(_d3.d3.merge(dom_ys));

    if (sublevel && mark.type === 'bar' && mark.arrange === 'stacked') {
      test.forEach(calcStartTotal);
      if (config.x.type === 'ordinal' || config.x.type === 'linear' && config.x.bin) {
        dom_y = _d3.d3.extent(test.map(function (m) {
          return m.total;
        }));
      }
      if (config.y.type === 'ordinal' || config.y.type === 'linear' && config.y.bin) {
        dom_x = _d3.d3.extent(test.map(function (m) {
          return m.total;
        }));
      }
  ...
  var filt1_dom_x = _d3.d3.extent(_d3.d3.merge(filt1_xs));
  var filt1_dom_y = _d3.d3.extent(_d3.d3.merge(filt1_ys));
  ...
  var x_dom = config.x_dom ? config.x_dom : config.x.type === 'ordinal' && config.x.behavior === 'flex' ? _d3.d3.set(filtered.map(function (m) {
    return m[config.x.column];
  })).values() : config.x.type === 'ordinal' ? _d3.d3.set(raw.map(function (m) {
    return m[config.x.column];
  })).values() : config.x_from0 ? [0, _d3.d3.max(pre_x_dom)] : pre_x_dom;

  var y_dom = config.y_dom ? config.y_dom : config.y.type === "ordinal" && config.y.behavior === 'flex' ? _d3.d3.set(filtered.map(function (m) {
    return m[config.y.column];
  })).values() : config.y.type === "ordinal" ? _d3.d3.set(raw.map(function (m) {
    return m[config.y.column];
  })).values() : config.y_from0 ? [0, _d3.d3.max(pre_y_dom)] : pre_y_dom;
...
function setMargins() {
  ...
  var y_ticks = this.yAxis.tickFormat() ? this.y.domain().map(function (m) {
    return _this2.yAxis.tickFormat()(m);
  }) : this.y.domain();

  var max_y_text_length = _d3.d3.max(y_ticks.map(function (m) {
...
function setColorScale() {
  ...
  var colordom = config.color_dom || _d3.d3.set(data.map(function (m) {
  ...
  if (config.legend.order) {
    colordom = colordom.sort(function (a, b) {
      return _d3.d3.ascending(config.legend.order.indexOf(a), config.legend.order.indexOf(b));
    });
  } else {
    colordom = colordom.sort(naturalSorter);
  }

  this.colorScale = _d3.d3.scale.ordinal().domain(colordom).range(config.colors);
}
...
function resize() {
  ...
  _d3.d3.select(this.svg.node().parentNode).attr('width', svg_width).attr('height', svg_height).select('g').attr('transform', 'translate(' + this.margin.left + ',' + this.margin.top + ')');
...
function makeLegend() {
  ...
  if (config.legend.order) {
    leg_parts.sort(function (a, b) {
      return _d3.d3.ascending(config.legend.order.indexOf(a.label), config.legend.order.indexOf(b.label));
    });
  }

  leg_parts.selectAll('.legend-color-block').select('.legend-mark').remove();
  leg_parts.selectAll('.legend-color-block').each(function (e) {
    var svg = _d3.d3.select(this);
    if (e.mark === 'circle') {
  ...
  leg_parts.selectAll('.legend-color-block').select('.legend-mark').attr('fill', function (d) {
    ...
    _d3.d3.select(this).attr(e.attributes);
  });
...
function layout() {
  ...
  _d3.d3.select(this.div).select('.loader').remove();
...
function init(data) {
  ...
  if (_d3.d3.select(this.div).select('.loader').empty()) {
    _d3.d3.select(this.div).insert('div', ':first-child').attr('class', 'loader').selectAll('.blockG').data(_d3.d3.range(8)).enter().append('div').attr('class', function (d) {
  ...
  var startup = function startup(data) {
    ...
    if (_this3.controls) {
      _this3.controls.targets.push(_this3);
      if (!_this3.controls.ready) {
        _this3.controls.init(_this3.raw_data);
      } else {
        _this3.controls.layout();
      }
    }

    //make sure container is visible (has height and width) before trying to initialize
    var visible = _d3.d3.select(_this3.div).property('offsetWidth') > 0;
    if (!visible) {
      ...
      var onVisible = setInterval(function (i) {
        var visible_now = _d3.d3.select(_this3.div).property('offsetWidth') > 0;
        if (visible_now) {
          _this3.layout();
          _this3.wrap.datum(_this3);
          _this3.draw();
...
function drawPoints(marks) {
  ...
  points.each(function (d) {
    var mark = _d3.d3.select(this.parentNode).datum();
    d.mark = mark;
    _d3.d3.select(this).select('circle').attr(mark.attributes);
  });
  //animated attributes
  ...
  pointsTrans.attr('r', function (d) {
    ...
    var x_pos = _this4.x(d.values.x) || 0;
    return config.x.type === 'ordinal' ? x_pos + _this4.x.rangeBand() / 2 : x_pos;
  }).attr('cy', function (d) {
    var y_pos = _this4.y(d.values.y) || 0;
    return config.y.type === 'ordinal' ? y_pos + _this4.y.rangeBand() / 2 : y_pos;
  });

  points.select('title').text(function (d) {
    var tt = d.mark.tooltip || '';
    var xformat = config.x.summary === 'percent' ? _d3.d3.format('0%') : config.x.type === 'time' ? _d3.d3.time.format(config.x.format) : _d3.d3.format(config.x.format);
    var yformat = config.y.summary === 'percent' ? _d3.d3.format('0%') : config.y.type === 'time' ? _d3.d3.time.format(config.y.format) : _d3.d3.format(config.y.format);
...
function drawLines(marks) {
  ...
  var config = this.config;
  var line = _d3.d3.svg.line().interpolate(config.interpolate).x(function (d) {
  ...
  line_grps.each(function (d) {
    ...
    _d3.d3.select(this).select('path').attr(mark.attributes);
  });

  line_grps.select('title').text(function (d) {
    var tt = d.tooltip || '';
    var xformat = config.x.summary === 'percent' ? _d3.d3.format('0%') : _d3.d3.format(config.x.format);
    var yformat = config.y.summary === 'percent' ? _d3.d3.format('0%') : _d3.d3.format(config.y.format);
...
function drawBars(marks) {
  ...
  if (config.x.type === 'ordinal') {
    (function () {
      oldBarsTrans.attr('y', _this6.y(0)).attr('height', 0);
      ...
      exitBars.attr('y', _this6.y(0)).attr('height', 0).remove();
      ...
      bars.each(function (d) {
        var mark = _d3.d3.select(this.parentNode.parentNode).datum();
        d.tooltip = mark.tooltip;
        d.arrange = mark.split ? mark.arrange : null;
        d.subcats = config.legend.order ? config.legend.order.slice().reverse() : mark.values && mark.values[mark.split] ? mark.values[mark.split] : _d3.d3.set(rawData.map(function (m) {
          return m[mark.split];
        })).values();
        _d3.d3.select(this).attr(mark.attributes);
      });

      var xformat = config.marks.map(function (m) {
        return m.summarizeX === 'percent';
      }).indexOf(true) > -1 ? _d3.d3.format('0%') : _d3.d3.format(config.x.format);
      var yformat = config.marks.map(function (m) {
        return m.summarizeY === 'percent';
      }).indexOf(true) > -1 ? _d3.d3.format('0%') : _d3.d3.format(config.y.format);
      ...
      barsTrans.attr('x', function (d) {
        ...
        if (d.arrange !== 'stacked') {
          return _this6.y(d.values.y);
        } else {
          return _this6.y(d.values.start);
        }
        ...
        return _this6.y(0) - _this6.y(d.values.y);
      });
    })();
  } else if (config.y.type === 'ordinal') {
    (function () {
      oldBarsTrans.attr('x', _this6.x(0)).attr('width', 0);
      ...
      var exitBars = config.transitions ? bars.exit().transition() : bars.exit();
      exitBars.attr('x', _this6.x(0)).attr('width', 0).remove();
      bars.enter().append('rect').attr('class', function (d) {
        return 'wc-data-mark bar ' + d.key;
      }).style('clip-path', 'url(#' + _this6.id + ')').attr('x', _this6.x(0)).attr('width', 0).append('title');
      ...
      barsTrans.attr('x', function (d) {
        if (d.arrange === 'stacked' || !d.arrange) {
          return d.values.start !== undefined ? _this6.x(d.values.start) : _this6.x(0);
        } else {
          return _this6.x(0);
        }
      }).attr('y', function (d) {
        if (d.arrange === 'nested') {
          var position = d.subcats.indexOf(d.key);
          var offset = position ? _this6.y.rangeBand() / (d.subcats.length * 0.75) / position : _this6.y.rangeBand();
          return _this6.y(d.values.y) + (_this6.y.rangeBand() - offset) / 2;
        } else if (d.arrange === 'grouped') {
          var position = d.subcats.indexOf(d.key);
          return _this6.y(d.values.y) + _this6.y.rangeBand() / d.subcats.length * position;
        } else {
          return _this6.y(d.values.y);
        }
      }).attr('width', function (d) {
        return _this6.x(d.values.x) - _this6.x(0);
      }).attr('height', function (d) {
        if (config.y.type === 'quantile') {
          return 20;
        } else if (d.arrange === 'nested') {
          var position = d.subcats.indexOf(d.key);
          return position ? _this6.y.rangeBand() / (d.subcats.length * 0.75) / position : _this6.y.rangeBand();
        } else if (d.arrange === 'grouped') {
          return _this6.y.rangeBand() / d.subcats.length;
        } else {
          return _this6.y.rangeBand();
        }
      });
    })();
    ...
    (function () {
      ...
      bars.each(function (d) {
        ...
        _d3.d3.select(this).attr(mark.attributes);
        var parent = _d3.d3.select(this.parentNode).datum();
        var rangeSet = parent.key.split(',').map(function (m) {
          return +m;
        });
        d.rangeLow = _d3.d3.min(rangeSet);
        d.rangeHigh = _d3.d3.max(rangeSet);
        d.tooltip = mark.tooltip;
      });
      ...
      barsTrans.attr('x', function (d) {
        ...
        return _this6.x(d.rangeHigh) - _this6.x(d.rangeLow);
      }).attr('height', function (d) {
        return _this6.y(0) - _this6.y(d.values.y);
      });
    })();
    ...
    (function () {
      ...
      barsTrans.attr('x', function (d) {
        if (d.arrange === 'stacked') {
          return _this6.x(d.values.start);
        } else {
          return _this6.x(0);
        }
      }).attr('y', function (d) {
        return _this6.y(d.rangeHigh);
      }).attr('width', function (d) {
        return _this6.x(d.values.x);
      }).attr('height', function (d) {
        return _this6.y(d.rangeLow) - _this6.y(d.rangeHigh);
      });
    })();
  } else {
...
function drawArea(area_drawer, area_data, datum_accessor, class_match, bind_accessor) {
  ...
  var areaPaths = area_grps.select('path').datum(datum_accessor).attr('fill', function (d) {
    ...
    return d_attr ? _this7.colorScale(d_attr[_this7.config.color_by]) : null;
...
function draw(raw_data, processed_data) {
  ...
  this.svg.select(".x.axis").select(".axis-title").text(function (d) {
    return typeof config.x.label === "string" ? config.x.label : typeof config.x.label === "function" ? config.x.label.call(_this8) : null;
  });
  this.svg.select(".y.axis").select(".axis-title").text(function (d) {
    return typeof config.y.label === "string" ? config.y.label : typeof config.y.label === "function" ? config.y.label.call(_this8) : null;
  });
  ...
  if (config.resizable && window) {
    _d3.d3.select(window).on('resize.' + context.element + context.id, function () {
      context.resize();
    });
  } else if (window) {
    _d3.d3.select(window).on('resize.' + context.element + context.id, null);
  }
...
function consolidateData(raw) {
  ...
  config.marks.forEach(function (e, i) {
    ...
    var mark_info = e.per ? _this9.transformData(raw, e) : { data: [], x_dom: [], y_dom: [] };

    all_data.push(mark_info.data);
    all_x.push(mark_info.x_dom);
    all_y.push(mark_info.y_dom);
    _this9.marks[i] = Object.create(e);
    _this9.marks[i].data = mark_info.data;
  ...
  if (config.x.type === 'ordinal') {
    if (config.x.domain) {
      ...
      this.x_dom = _d3.d3.set(_d3.d3.merge(all_x)).values().sort(function (a, b) {
        return _d3.d3.ascending(config.x.order.indexOf(a), config.x.order.indexOf(b));
      });
    } else if (config.x.sort && config.x.sort === 'alphabetical-ascending') {
      this.x_dom = _d3.d3.set(_d3.d3.merge(all_x)).values().sort(naturalSorter);
    } else if (config.y.type === 'time' && config.x.sort === 'earliest') {
      this.x_dom = _d3.d3.nest().key(function (d) {
        ...
        return _d3.d3.min(b.values) - _d3.d3.min(a.values);
      }).map(function (m) {
        return m.key;
      });
      ...
      this.x_dom = _d3.d3.set(_d3.d3.merge(all_x)).values();
    }
    ...
    this.x_dom = _d3.d3.extent(_d3.d3.merge(all_x));
  }
  ...
  if (config.y.type === 'ordinal') {
    if (config.y.domain) {
      ...
      this.y_dom = _d3.d3.set(_d3.d3.merge(all_y)).values().sort(function (a, b) {
        return _d3.d3.ascending(config.y.order.indexOf(a), config.y.order.indexOf(b));
      });
    } else if (config.y.sort && config.y.sort === 'alphabetical-ascending') {
      this.y_dom = _d3.d3.set(_d3.d3.merge(all_y)).values().sort(naturalSorter);
    } else if (config.x.type === 'time' && config.y.sort === 'earliest') {
      this.y_dom = _d3.d3.nest().key(function (d) {
      ...
      this.y_dom = _d3.d3.set(_d3.d3.merge(all_y)).values().sort(naturalSorter).reverse();
    } else {
      this.y_dom = _d3.d3.set(_d3.d3.merge(all_y)).values();
    }
  } else if (config.marks.map(function (m) {
    return m.summarizeY === 'percent';
  }).indexOf(true) > -1) {
    ...
    this.y_dom = _d3.d3.extent(_d3.d3.merge(all_y));
  }
}
...
function checkRequired(data) {
  ...
  requiredCols.forEach(function (e, i) {
    if (colnames.indexOf(e) < 0) {
      _d3.d3.select(_this10.div).select('.loader').remove();
      _this10.wrap.append('div').style('color', 'red').html('The value "' + e + '" for the <code>' + requiredVars[i] + '</code> setting does not match any column in the provided dataset.');
...
function createChart() {
  ...
  thisChart.wrap = _d3.d3.select(thisChart.div).append('div');
...
function multiply(chart, data, split_by, order) {
  ...
  function goAhead(data) {
    var split_vals = _d3.d3.set(data.map(function (m) {
      return m[split_by];
    })).values().filter(function (f) {
      return f;
    });
    if (order) {
      split_vals = split_vals.sort(function (a, b) {
        return _d3.d3.ascending(order.indexOf(a), order.indexOf(b));
      });
    }
...
function draw$1(raw_data, processed_data) {
  ...
  var col_list = config.cols.length ? config.cols : data.length ? _d3.d3.keys(data[0].values[0].raw) : [];
...
function transformData$1(data) {
  ...
  var config = this.config;
  var colList = config.cols || _d3.d3.keys(data[0]);
  if (config.keep) {
  ...
  var slimmed = _d3.d3.nest().key(function (d) {
    if (config.row_per) {
...
function createTable() {
  ...
  thisTable.wrap = _d3.d3.select(thisTable.div).append('div');
...
function makeTextControl(control, control_wrap) {
  ...
  var changer = control_wrap.append('input').attr('type', 'text').attr('class', 'changer').datum(control).property('value', function (d) {
    return _this11.stringAccessor(_this11.targets[0].config, control.option);
  });

  changer.on('change', function (d) {
    var value = changer.property('value');
    _this11.changeOption(control.option, value, control.callback);
  });
}
...
function makeSubsetterControl(control, control_wrap) {
  ...
  var option_data = control.values ? control.values : _d3.d3.set(this.data.map(function (m) {
    ...
    if (control.multiple) {
      (function () {
        var values = options.filter(function (f) {
          return _d3.d3.select(this).property('selected');
        })[0].map(function (m) {
          return _d3.d3.select(m).property('text');
        });
      ...
      (function () {
        var value = _d3.d3.select(_this12).select("option:checked").property('text');
...
function makeRadioControl(control, control_wrap) {
  ...
  var changers = control_wrap.selectAll('label').data(control.values || _d3.d3.keys(this.data[0])).enter().append('label').attr('class', 'radio').text(function (d, i) {
  ...
  changers.on('change', function (d) {
    ...
    changers.each(function (c) {
      if (_d3.d3.select(this).property('checked')) {
        value = _d3.d3.select(this).property('value') === 'none' ? null : c;
      }
    });
    _this13.changeOption(control.option, value, control.callback);
  });
}
...
function makeNumberControl(control, control_wrap) {
  ...
  changer.on('change', function (d) {
    var value = +changer.property('value');
    _this14.changeOption(control.option, value, control.callback);
  });
}
...
function makeListControl(control, control_wrap) {
  ...
  changer.on('change', function (d) {
    ...
    _this15.changeOption(control.option, value, control.callback);
  });
}
...
function makeDropdownControl(control, control_wrap) {
  ...
  var opt_values = control.values && control.values instanceof Array ? control.values : control.values ? _d3.d3.set(this.data.map(function (m) {
    return m[_this16.targets[0].config[control.values]];
  })).values() : _d3.d3.keys(this.data[0]);
  ...
  changer.on('change', function (d) {
    ...
    if (control.multiple) {
      value = options.filter(function (f) {
        ...
        return _d3.d3.select(m).property('value');
      }).filter(function (f) {
        return f !== 'None';
      });
    }
    ...
    if (control.options) {
      _this16.changeOption(control.options, value, control.callback);
    } else {
      _this16.changeOption(control.option, value, control.callback);;
    }
  });
...
function makeCheckboxControl(control, control_wrap) {
  ...
  changer.on('change', function (d) {
    ...
    _this17.changeOption(d.option, value, control.callback);
  });
}
...
function makeBtnGroupControl(control, control_wrap) {
  ...
  var option_data = control.values ? control.values : _d3.d3.keys(this.data[0]);
  ...
  changers.on('click', function (d) {
    changers.each(function (e) {
      _d3.d3.select(this).classed('btn-primary', e === d);
    });
    _this18.changeOption(control.option, d, control.callback);
  });
}
...
function checkRequired$1(dataset) {
  ...
  var colnames = _d3.d3.keys(dataset[0]);
  ...
  this.targets.forEach(function (e) {
    if (option instanceof Array) {
      option.forEach(function (o) {
        return _this20.stringAccessor(e.config, o, value);
      });
    } else {
      _this20.stringAccessor(e.config, option, value);
    }
    //call callback function if provided
    if (callback) {
      callback();
...
function createControls() {
  ...
  if (config.location === 'bottom') {
    thisControls.wrap = _d3.d3.select(element).append('div').attr('class', 'wc-controls');
  } else {
    thisControls.wrap = _d3.d3.select(element).insert('div', ':first-child').attr('class', 'wc-controls');
  }
...
function getValType(data, variable) {
  var var_vals = _d3.d3.set(data.map(function (m) {
    return m[variable];
  })).values();
