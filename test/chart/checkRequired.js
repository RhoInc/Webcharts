...
import checkRequired from '../../src/chart/checkRequired';
...
describe('upon chart initialization data field arguments are checked against actual fields in data', () => {
  ...
  //Chart initialization
    ...
    it('checkRequired() returns a boolean corresponding to whether or not the data contain all required data fields', () => {
        const
            expected = verifyRequired(settings).map(item => item.dataField).some(dataField => Object.keys(data[0]).indexOf(dataField) === -1),
    ...
    it('checkRequired() returns a list of settings arguments associated with required data fields', () => {
        const
            expectedArguments = verifyRequired(settings).map(item => item.argument).sort(),
            actualArguments = chart.checkRequired(data).dataFieldArguments.map(argument => argument.replace('this.config.', '')).sort();
        expect(expectedArguments)
            .toEqual(actualArguments);
    });

    it('checkRequired() returns a list of required data fields', () => {
        const
            expectedDataFields = d3.set(verifyRequired(settings).map(item => item.dataField)).values(),
            actualDataFields = d3.set(chart.checkRequired(data).requiredDataFields).values();
        expect(expectedDataFields)
            .toEqual(actualDataFields);
    });
});
