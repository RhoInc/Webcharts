    ...
    describe('transformData() is called for each item in settings.marks', () => {
        settings.marks.forEach(mark => {
            ...
            beforeEach(() => {
                ...
                nestedData = d3.nest() // validation dataset
                    ...
                    .rollup(d => {
                        return {
                            ...
                            x_q25: d3.quantile(d.map(di => di[settings.x.column]), .25),
                            x_q75: d3.quantile(d.map(di => di[settings.x.column]), .75),
                            y_q25: d3.quantile(d.map(di => di[settings.y.column]), .25),
                            y_q75: d3.quantile(d.map(di => di[settings.y.column]), .75)};
                    })
                    .entries(data);
            });
            ...
            it('sets x-domain to extent of values in x-column', () => {
                expect(Math.min.apply(null,nestedData.map(d => d.values.x))).toEqual(transformedData.x_dom[0]);
                expect(Math.max.apply(null,nestedData.map(d => d.values.x))).toEqual(transformedData.x_dom[1]);
            });

            it('sets y-domain to extent of values in y-column', () => {
                expect(Math.min.apply(null,nestedData.map(d => d.values.y))).toEqual(transformedData.y_dom[0]);
                expect(Math.max.apply(null,nestedData.map(d => d.values.y))).toEqual(transformedData.y_dom[1]);
            });
        });
    });
