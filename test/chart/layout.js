...
export default function testLayout(settings,data) {
    describe('chart layout', () => {
        ...
        it('creates a defs child element of svg with a clipPath child element', () => {
            const
                defs = svgChildren
                    .filter(child => child.nodeName.toLowerCase() === 'defs'),
                ...
                clipPath = defsChildren.filter(child => child.nodeName.toLowerCase() === 'clippath');
        ...
        it('creates an x-axis g child element of svg', () => {
            const xAxis = svgChildren
                .filter(child => child.nodeName.toLowerCase() === 'g' && child.className.indexOf('x axis') > -1);
            expect(xAxis.length).toEqual(1);
        });

        it('creates a y-axis g child element of svg', () => {
            const yAxis = svgChildren
                .filter(child => child.nodeName.toLowerCase() === 'g' && child.className.indexOf('y axis') > -1);
            expect(yAxis.length).toEqual(1);
        });

        it('creates an overlay rect child element of svg', () => {
            const overlay = svgChildren
                .filter(child => child.nodeName.toLowerCase() === 'rect' && child.className.indexOf('overlay') > -1);
            expect(overlay.length).toEqual(1);
        });

        it('creates a legend ul child element of chart container', () => {
            const legend = Array.prototype.slice.call(chart.wrap.node().children)
                .filter(child => child.nodeName.toLowerCase() === 'ul' && child.className.indexOf('legend') > -1);
