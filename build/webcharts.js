...
function applyFilters() {
    ...
    this.data.filtered = this.filters ? clone(this.data.raw).filter(function (d) {
        ...
        _this.filters.forEach(function (filter) {
            if (match === false && filter.val !== 'All') match = filter.val instanceof Array ? filter.val.indexOf(d[filter.col]) > -1 : filter.val === d[filter.col];
        });
...
function draw$1(passed_data) {
    ...
    //Apply pagination.
    if (this.data.paginated) data = data.filter(function (d, i) {
        return _this.config.startIndex <= i && i < _this.config.endIndex;
    });

    //Define column headers.
    var headers = this.thead.append('tr').selectAll('th').data(this.config.headers);
    headers.exit().remove();
    ...
    if (this.config.sortable) headers.on('click', function (header) {
    ...
    if (data.length === 0) this.tbody.append('tr').attr('class', 'NoDataRow').text('No data selected.');else {
        ...
        var cells = rows.selectAll('td').data(function (d) {
            return Object.keys(d).filter(function (key) {
                return _this.config.cols.indexOf(key).map(function (key) {
                    return d[key];
                });
            });
        });
...
function addArrows() {
    ...
    this.pagination.prev = this.pagination.wrap.insert('a', ':first-child').classed('left arrow-link', true).classed('hidden', this.config.activePage == 0).attr({
        rel: prev
    }).text('<');

    this.pagination.doublePrev = this.pagination.wrap.insert('a', ':first-child').classed('left double-arrow-link', true).classed('hidden', this.config.activePage == 0).attr({
        rel: 0
    }).text('<<');
    ...
    this.pagination.next = this.pagination.wrap.append('a').classed('right arrow-link', true).classed('hidden', this.config.activePage == this.config.nPages - 1 || this.config.nPages == 0).attr({
        rel: next
    }).text('>');

    this.pagination.doubleNext = this.pagination.wrap.append('a').classed('right double-arrow-link', true).classed('hidden', this.config.activePage == this.config.nPages - 1 || this.config.nPages == 0).attr({
