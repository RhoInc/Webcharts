...
function setDomain(axis) {
    ...
    if (this.config[axis].type === 'ordinal') {
        if (this.config[axis].domain) {
            ...
            this[axis + '_dom'] = d3.set(d3.merge(all_x)).values().sort(function (a, b) {
            ...
            this[axis + '_dom'] = d3.set(d3.merge(all_x)).values().sort(naturalSorter);
            ...
            this[axis + '_dom'] = d3.set(d3.merge(all_x)).values();
        }
        ...
        this[axis + '_dom'] = d3.extent(d3.merge(all_x));
    }

    //Give the domain a range when the range of the variable is 0.
    if (this.config[axis].type === 'linear' && this[axis + '_dom'][0] === this[axis + '_dom'][1]) this[axis + '_dom'] = [this[axis + '_dom'][0] - this[axis + '_dom'][0] * .01, this[axis + '_dom'][1] + this[axis + '_dom'][1] * .01];
}
...
function consolidateData(raw) {
    ...
    //}
    //if (this.x_dom[0] === this.x_dom[1])
    //    this.x_dom = [
    //        this.x_dom[0] - this.x_dom[0]*.01,
    //        this.x_dom[1] + this.x_dom[1]*.01
    //    ];
    ...
    //    this.y_dom = [
    //        this.y_dom[0] - this.y_dom[0]*.01,
    //        this.y_dom[1] + this.y_dom[1]*.01
    //    ];
}
    ...
    function makeNest(entries, sublevel) {
        ...
        if (sublevel) {
            ...
            this_nest.sortKeys(function (a, b) {
                return config.x.type === 'time' ? d3.ascending(new Date(a), new Date(b)) : config.x.order ? d3.ascending(config.x.order.indexOf(a), config.x.order.indexOf(b)) : sublevel === config.color_by && config.legend.order ? d3.ascending(config.legend.order.indexOf(a), config.legend.order.indexOf(b)) : config.x.type === 'ordinal' || config.y.type === 'ordinal' ? naturalSorter$1(a, b) : d3.ascending(+a, +b);
            });
        }
...
function draw$1(passed_data) {
    ...
    this.searchable.wrap.select('.nNrecords').text(data.length === this.data.raw.length ? this.data.raw.length + ' records displayed' : data.length + '/' + this.data.raw.length + ' records displayed');
...
function csv(data) {
    ...
    var CSV = new Blob([CSVarray.join('\n')], { type: 'text/csv;charset=utf-8;' }),
        fileName = 'webchartsTableExport_' + d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()) + '.csv',
...
function xlsx(data) {
    ...
    var blob = new Blob([s2ab(xlsx)], { type: 'application/octet-stream;' }),
        fileName = 'webchartsTableExport_' + d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()) + '.xlsx',
