...
function init(data) {
    ...
    if (d3$1.select(this.div).select('.loader').empty()) {
        d3$1.select(this.div).insert('div', ':first-child').attr('class', 'loader').selectAll('.blockG').data(d3$1.range(8)).enter().append('div').attr('class', function (d) {
...
function applyFilters() {
    ...
    this.data.filtered = this.filters ? clone(this.data.raw).filter(function (d) {
        var match = true;
...
function draw$1(passed_data) {
    ...
    this.searchable.wrap.select('.nNrecords').classed('invisible', data.length === this.data.raw.length).text(data.length + '/' + this.data.raw.length + ' records displayed');
...
function layout$3() {
    ...
    if (this.config.exports && this.config.exports.length) this.config.exports.forEach(function (fmt) {
        _this.exportable.wrap.append('a').classed('export', true).attr({
            id: fmt
...
function xlsx(data) {
    ...
    workbook.Sheets[sheetName] = XLSX.utils.aoa_to_sheet([this.config.headers].concat(arrayOfArrays));
    ...
    var blob = new Blob([s2ab(xlsx)], { type: 'application/octet-stream;' }),
        fileName = 'webhartsTableExport_' + d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()) + '.xlsx',
...
function layout$4() {
    ...
    this.sortable.wrap.append('span').classed('instruction', true).text('Click any column header to sort that column.');
}
...
function onClick(th, header) {
    ...
    if (!sortItem) {
        sortItem = {
            col: col,
            direction: 'ascending',
            wrap: this.sortable.wrap.append('div').datum({ key: col }).classed('sort-box', true).text(col)
        };
...
function addArrows() {
    ...
    this.pagination.wrap.insert('span', ':first-child').classed('dot-dot-dot', true).text('...').classed('invisible', this.config.activePage < this.config.nPageLinksDisplayed);

    this.pagination.prev = this.pagination.wrap.insert('a', ':first-child').classed('left arrow-link', true).classed('invisible', this.config.activePage == 0).attr({
        rel: prev
    }).text('<');

    this.pagination.doublePrev = this.pagination.wrap.insert('a', ':first-child').classed('left double-arrow-link', true).classed('invisible', this.config.activePage == 0).attr({
        rel: 0
    }).text('<<');
    ...
    this.pagination.wrap.append('span').classed('dot-dot-dot', true).text('...').classed('invisible', this.config.activePage >= Math.max(this.config.nPageLinksDisplayed, this.config.nPages - this.config.nPageLinksDisplayed) || this.config.nPages <= this.config.nPageLinksDisplayed);
    this.pagination.next = this.pagination.wrap.append('a').classed('right arrow-link', true).classed('invisible', this.config.activePage == this.config.nPages - 1 || this.config.nPages == 0).attr({
        rel: next
    }).text('>');

    this.pagination.doubleNext = this.pagination.wrap.append('a').classed('right double-arrow-link', true).classed('invisible', this.config.activePage == this.config.nPages - 1 || this.config.nPages == 0).attr({
...
function layout$6() {
    ...
    this.thead.append('tr').selectAll('th').data(this.config.headers).enter().append('th').attr('class', function (d) {
