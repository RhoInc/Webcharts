...
function layout$2() {
    ...
    this.sort.wrap.append('span').classed('instruction', true).text('Click any column header to sort that column.');
}
...
function onClick(th, header) {
    ...
    if (!sortItem) {
        sortItem = {
            col: col,
            direction: 'ascending',
            wrap: this.sort.wrap.append('div').datum({ key: col }).classed('sort-box', true).text(col)
        };
    ...
    this.sort.order.forEach(function (item, i) {
        item.wrap.on('click', function (d) {
            ...
            context.sort.order.splice(context.sort.order.map(function (d) {
                return d.col;
            }).indexOf(d.key), 1);
...
function addArrows() {
    ...
    this.pagination.next = this.pagination.wrap.append('a').classed('right arrow-link', true).classed("hidden", this.config.activePage == this.config.nPages - 1).attr({
        rel: next
    }).text('>');

    this.pagination.doubleNext = this.pagination.wrap.append('a').classed('right double-arrow-link', true).classed("hidden", this.config.activePage == this.config.nPages - 1).attr({
