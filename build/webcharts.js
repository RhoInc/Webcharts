(function (global, factory) {
    ...
    function makeNest(mark, entries, sublevel) {
        ...
        if (this.config.x.type === 'linear' && this.config.x.bin || this.config.y.type === 'linear' && this.config.y.bin) {
            ...
            var quant = d3.scale.quantile().domain(this.config[xy].behavior !== 'flex' && this.config[xy].domain ? this.config[xy].domain : d3.extent(entries.map(function (m) {
    ...
    function drawLines(marks) {
        ...
        var linePaths = line_grps.select('path').attr('class', 'wc-data-mark').style('clip-path', 'url(#' + chart.id + ')').datum(function (d) {
            return d.values;
        }).attr('stroke', function (d) {
    ...
    function drawPoints(marks) {
        ...
        //static attributes
        points.select('circle').style('clip-path', 'url(#' + chart.id + ')').attr('fill-opacity', config.fill_opacity || config.fill_opacity === 0 ? config.fill_opacity : 0.6).attr('fill', function (d) {
            return _this.colorScale(d.values.raw[0][config.color_by]);
        }).attr('stroke', function (d) {
            return _this.colorScale(d.values.raw[0][config.color_by]);
        });
    ...
    function makeSubsetterControl(control, control_wrap) {
        ...
        //dropdown selection
        var changer = control_wrap.append('select').classed('changer', true).attr('multiple', control.multiple ? true : null).datum(control);
