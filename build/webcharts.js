(function (global, factory) {
    ...
    function setDomain(axis) {
      ...
      if (thisAxis.type === 'ordinal') {
        ...
        if (thisAxis.domain) {
          ...
          dom = d3.set(d3.merge(this.marks.map(function (mark) {
        ...
        //TODO: raw data
        dom = d3.extent(d3.merge(this.marks.map(function (mark) {
          return mark[axis + '_dom'];
        })));
      } //Give the domain a range when the range of the variable is 0.


      if (thisAxis.type === 'linear' && dom[0] === dom[1]) dom = dom[0] !== 0 ? [dom[0] - dom[0] * 0.01, dom[1] + dom[1] * 0.01] : [-1, 1];
      this[axis + '_dom'] = dom;
      return dom;
    }
      ...
      if (this.filters.length) {
        this.filters.forEach(function (filter) {
          _this.filtered_data = _this.filtered_data.filter(function (d) {
            return filter.all === true && filter.index === 0 ? d : filter.val instanceof Array ? filter.val.indexOf(d[filter.col]) > -1 : d[filter.col] + '' === filter.val + '';
          });
        });
    ...
    function makeNest(mark, entries, sublevel) {
      ...
      if (this.config.x.type === 'linear' && this.config.x.bin || this.config.y.type === 'linear' && this.config.y.bin) {
        ...
        mark.quant = d3.scale.quantile().domain(this.config[xy].domain ? this.config[xy].domain : d3.extent(entries.map(function (m) {
      ...
      if (sublevel) {
        ...
        this_nest.sortKeys(function (a, b) {
          ...
          if (_this.config.x.type === 'time' || _this.config.y.type === 'time') {
            sort = d3.ascending(new Date(a), new Date(b));
          } else if (sublevel === _this.config.x.column && _this.config.x.order) {
            sort = d3.ascending(_this.config.x.order.indexOf(a), _this.config.x.order.indexOf(b)); // lines
          } else if (sublevel === _this.config.y.column && _this.config.y.order) {
            sort = d3.ascending(_this.config.y.order.indexOf(a), _this.config.y.order.indexOf(b)); // lines
          } else if (sublevel === _this.config.color_by && _this.config.legend.order) {
            sort = d3.ascending(_this.config.legend.order.indexOf(a), _this.config.legend.order.indexOf(b)); // arranged bars
      ...
      } else totalOrder = test.map(function (m) {
        return m.key;
      });
    ...
    function transformData(raw, mark) {
      ...
      if (this.filters.length) {
        this.filters.forEach(function (e) {
          filtered = filtered.filter(function (d) {
            return e.all === true && e.index === 0 ? d : e.val instanceof Array ? e.val.indexOf(d[e.col]) > -1 : d[e.col] + '' === e.val.toString() + '';
          });
      ...
      var y_dom = config.y_dom ? config.y_dom : config.y.type === 'ordinal' && config.y.behavior === 'flex' ? d3.set(filtered.map(function (m) {
        ...
        return m[config.y.column];
      })).values() : pre_y_dom; //set lower limit of linear domain to 0 when other axis is ordinal and mark type is set to 'bar', provided no values are negative
    ...
    function moveLegend(scale) {
      ...
      if (!this.parent) {
        ...
        if (this.config.legend.location === 'top' || this.config.legend.location === 'left') {
          this.parent.wrap.node().insertBefore(legend.node(), this.parent.wrap.select('.wc-chart').node());
        } else {
    ...
    function defineLegendData(custom_data, scale) {
      ...
      var legend_data = Array.isArray(custom_data) && custom_data.length ? custom_data : scale.domain().slice(0).filter(function (f) {
    ...
    function addLegendItems(legend_data, scale) {
      ...
      var legend_items = all_legend_items.enter().append('li').classed('legend-item', true); // update order of legend items in DOM

      if (this.config.legend.order) {
        legend_items.sort(function (a, b) {
          return d3.ascending(_this.config.legend.order.indexOf(a.label), _this.config.legend.order.indexOf(b.label));
        });
      }
    ...
    function drawArea(area_drawer, area_data, datum_accessor) {
      ...
      }).attr('fill-opacity', this.config.fill_opacity || this.config.fill_opacity === 0 ? this.config.fill_opacity : 0.3); //don't transition if config says not to
    ...
    function xOrdinal(oldBarsTrans, oldBarGroupsTrans, nu_bar_groups, bar_groups, bars) {
      ...
      bars = bar_groups.selectAll('rect').data(function (d) {
        ...
        return d.values instanceof Array ? d.values.sort(function (a, b) {
          return _this.colorScale.domain().indexOf(a.key) - _this.colorScale.domain().indexOf(b.key);
      ...
      }).style('clip-path', "url(#".concat(chart.id, ")")).attr('y', this.y(0)).attr('height', 0).append('title'); // sort bars in DOM to display widest bar behind every other bar and narrowest bar in front of every other bar - that's poorly worded but you get the gist
      ...
      bars.each(function (d) {
        ...
        d.subcats = config.legend.order ? config.legend.order.slice() : mark.values && mark.values[mark.split] ? mark.values[mark.split] : d3.set(rawData.map(function (m) {
          return m[mark.split];
        })).values().sort(); // controls the order of the bars in the chart
    ...
    function yOrdinal(oldBarsTrans, oldBarGroupsTrans, nu_bar_groups, bar_groups, bars) {
      ...
      bars.enter().append('rect').attr('class', function (d) {
        return 'wc-data-mark bar ' + d.key;
      }).style('clip-path', "url(#".concat(chart.id, ")")).attr('x', this.x(0)).attr('width', 0).append('title'); // sort bars in DOM to display widest bar behind every other bar and narrowest bar in front of every other bar - that's poorly worded but you get the gist
      ...
      barsTrans.attr('x', function (d) {
        ...
        if (d.arrange === 'nested') {
          ...
          return _this.y(d.values.y) + _this.y.rangeBand() / d.subcats.length * _position;
        } else {
          return _this.y(d.values.y);
        }
      }).attr('width', function (d) {
    ...
    function xBin(oldBarsTrans, oldBarGroupsTrans, nu_bar_groups, bar_groups, bars) {
      ...
      bars.enter().append('rect').attr('class', function (d) {
        return 'wc-data-mark bar ' + d.key;
      }).style('clip-path', "url(#".concat(chart.id, ")")).attr('y', this.y(0)).attr('height', 0).append('title');
      bars.attr('shape-rendering', 'crispEdges').attr('stroke', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
      }).attr('fill', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
    ...
    function yBin(oldBarsTrans, oldBarGroupsTrans, nu_bar_groups, bar_groups, bars) {
      ...
      bars.enter().append('rect').attr('class', function (d) {
        return 'wc-data-mark bar ' + d.key;
      }).style('clip-path', "url(#".concat(chart.id, ")")).attr('x', this.x(0)).attr('width', 0).append('title');
      bars.attr('shape-rendering', 'crispEdges').attr('stroke', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
      }).attr('fill', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
    ...
    function drawLines(marks) {
      ...
      var linePaths = line_grps.select('path').attr('class', 'wc-data-mark').style('clip-path', "url(#".concat(chart.id, ")")).datum(function (d) {
        return d.values;
    ...
    function drawPoints(marks) {
      ...
      points.select('circle').style('clip-path', "url(#".concat(chart.id, ")")).attr('fill-opacity', config.fill_opacity || config.fill_opacity === 0 ? config.fill_opacity : 0.6).attr('fill', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
      }).attr('stroke', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
      }); // attach mark info
      ...
      if (marks.length) {
        ...
        this.svg.select('.plotting-area').attr('width', this.plot_width + radius * 2 + 2) // plot width + circle radius * 2 + circle stroke width * 2
        .attr('height', this.plot_height + radius * 2 + 2) // plot height + circle radius * 2 + circle stroke width * 2
        .attr('transform', 'translate(-' + (radius + 1) + // translate left circle radius + circle stroke width
    ...
    function drawText(marks) {
      ...
      text_supergroups.enter().append('g').attr('class', function (d) {
        return 'supergroup text-supergroup ' + d.id;
      });
      text_supergroups.exit().remove();
      var texts = text_supergroups.selectAll('.text').data(function (d) {
        return d.data;
      }, function (d) {
        return d.key;
      });
      ...
      texts.select('text').style('clip-path', "url(#".concat(chart.id, ")")).attr('fill', function (d) {
        return _this.colorScale(d.values.raw[0][config.color_by]);
      }).text(function (d) {
        var tt = d.mark.text || '';
        var xformat = config.x.summary === 'percent' ? d3.format('0%') : config.x.type === 'time' ? d3.time.format(config.x.format) : d3.format(config.x.format);
    ...
    function checkRequired$1(dataset) {
      ...
      this.config.inputs.forEach(function (input, i) {
        if (input.type === 'subsetter' && colNames.indexOf(input.value_col) === -1) throw new Error("Error in settings object: the value \"".concat(input.value_col, "\" does not match any column in the provided dataset.")); //Draw the chart when a control changes unless the user specifies otherwise.
    ...
    function makeControlItem(control) {
      var control_wrap = this.wrap.append('div').attr('class', 'control-group').classed('inline', control.inline).datum(control); //Add control label span.

      var ctrl_label = control_wrap.append('span').attr('class', 'wc-control-label').text(control.label); //Add control _Required_ text to control label span.

      if (control.required) ctrl_label.append('span').attr('class', 'label label-required').text('Required'); //Add control description span.
    ...
    function makeSubsetterControl(control, control_wrap) {
      ...
      var changer = control_wrap.append('select').classed('changer', true).attr('multiple', control.multiple ? true : null).datum(control); //dropdown option data

      var option_data = control.values ? control.values : d3.set(this.data.map(function (m) {
        return m[control.value_col];
      })) //.filter(f => f))
      .values().sort(naturalSorter); // only sort when values are derived
    ...
    function applyFilters() {
      ...
      if (this.filters && this.filters.some(function (filter) {
        return typeof filter.val === 'string' && !(filter.all === true && filter.index === 0) || Array.isArray(filter.val) && filter.val.length < filter.choices.length;
      })) {
    ...
    function drawTableBody() {
      ...
      var rows = this.tbody.selectAll('tr').data(this.data.processing).enter().append('tr'); //Define table body cells.
    ...
    function draw$1(passed_data) {
      ...
      this.events.onPreprocess.call(this);
      if (!passed_data) //Apply filters if data is not passed to table.draw().
        applyFilters.call(this); //Otherwise update data object.
      ...
      this.searchable.wrap.select('.nNrecords').text(this.data.processing.length === this.data.raw.length ? "".concat(this.data.raw.length, " records displayed") : "".concat(this.data.processing.length, "/").concat(this.data.raw.length, " records displayed")); //Update table headers.
      ...
      if (this.data.processing.length === 0) {
        this.tbody.append('tr').classed('no-data', true).append('td').attr('colspan', this.config.cols.length).text('No data selected.'); //Bind table filtered/searched data to table container.
    ...
    function download(fileType, data) {
      ...
      var fileName = "webchartsTableExport_".concat(d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()), ".").concat(fileType);
    ...
    function xlsx(data) {
      ...
      this.wrap.select('.export#xlsx').on('click', function () {
        ...
        workbook.Sheets[sheetName] = XLSX.utils.aoa_to_sheet([_this.config.headers].concat(arrayOfArrays)); //Add filters to spreadsheet.

        workbook.Sheets[sheetName]['!autofilter'] = {
          ref: "A1:".concat(String.fromCharCode(64 + _this.config.cols.length)).concat(data.length + 1)
    ...
    function onClick(th, header) {
      ...
      if (!sortItem) {
        sortItem = {
          col: col,
          direction: 'ascending',
          wrap: this.sortable.wrap.append('div').datum({
            key: col
          }).classed('wc-button sort-box', true).text(header),
      ...
      this.sortable.order.forEach(function (item, i) {
        item.wrap.on('click', function (d) {
          ...
          context.sortable.order.splice(context.sortable.order.map(function (d) {
            return d.col;
          }).indexOf(d.key), 1); //Display sorting instruction.

          context.sortable.wrap.select('.instruction').classed('hidden', context.sortable.order.length); //Redraw chart.
    ...
    function addLinks() {
      ...
      var _loop = function _loop(i) {
        _this.pagination.wrap.append('a').datum({
          rel: i
        }).attr({
          rel: i
    ...
    function layout$6() {
      ...
      d3.select(this.div).select('.loader').remove(); //Attach container before table.
    ...
    function setDefaults$1(firstItem) {
      ...
      // cols
      if (!Array.isArray(this.config.cols) || Array.isArray(this.config.cols) && this.config.cols.length === 0) this.config.cols = d3.keys(firstItem); // headers

      if (!Array.isArray(this.config.headers) || Array.isArray(this.config.headers) && this.config.headers.length === 0 || Array.isArray(this.config.headers) && this.config.headers.length !== this.config.cols.length) this.config.headers = this.config.cols.slice(); // types

      if (_typeof(this.config.types) !== 'object') this.config.types = {};
      this.config.cols.forEach(function (col) {
        if (!['string', 'number'].includes(_this.config.types[col])) _this.config.types[col] = 'string';
    ...
    function transformData$1(processed_data) {
      ...
      this.data.current = slimmed.length ? slimmed : [{
        key: null,
        values: []
