(function (global, factory) {
    ...
    function positionLegend() {
      var legend;
      if (!this.hasOwnProperty('legend')) legend = this.wrap.select('.legend');else legend = this.legend;
    ...
    function legendTitle(label) {
      var legendTitle = this.legend.select('.legend-title').text(label ? label : typeof this.config.legend.label === 'string' ? this.config.legend.label : '');
      return legendTitle;
    }

    function defineMark() {
      var mark = this.config.legend.mark ? this.config.legend.mark : this.config.marks.length && this.config.marks[0].type === 'bar' ? 'square' : this.config.marks.length ? this.config.marks[0].type : 'square';
      return mark;
    }
    ...
    function makeLegend() {
      ...
      this.legend.mark = defineMark.call(this); // legend-item

      var leg_parts = this.legend.selectAll('.legend-item').data(legend_data, function (d) {
        return d.label + d.mark;
      });
      leg_parts.exit().remove();
      var new_parts = leg_parts.enter().append('li').classed('legend-item', true); // legend-mark-text
      ...
      leg_parts.classed('legend-item--display-block', !['bottom', 'top'].includes(this.config.legend.location));
    ...
    function drawText(marks) {
      ...
      texts.select('text').style('clip-path', "url(#".concat(chart.id, ")")).text(function (d) {
        var tt = d.mark.text || '';
        var xformat = config.x.summary === 'percent' ? d3.format('0%') : config.x.type === 'time' ? d3.time.format(config.x.format) : d3.format(config.x.format);
        var yformat = config.y.summary === 'percent' ? d3.format('0%') : config.y.type === 'time' ? d3.time.format(config.y.format) : d3.format(config.y.format);
        return tt.replace(/\$x/g, config.x.type === 'time' ? xformat(new Date(d.values.x)) : xformat(d.values.x)).replace(/\$y/g, config.y.type === 'time' ? yformat(new Date(d.values.y)) : yformat(d.values.y)).replace(/\[(.+?)\]/g, function (str, orig) {
