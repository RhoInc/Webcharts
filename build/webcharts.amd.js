...
define(['d3'], function (d3$1) {
  ...
  function drawPoints(marks) {
    ...
    //animated attributes
    points.select('circle').transition().attr('r', function (d) {
  ...
  function drawBars(marks) {
    ...
    if (config.x.type === 'ordinal') {
      (function () {
        old_bar_groups.selectAll('.bar').transition().attr('y', _this6.y(0)).attr('height', 0);
      ...
      (function () {
        old_bar_groups.selectAll('.bar').transition().attr('x', _this6.x(0)).attr('width', 0);
        old_bar_groups.transition().remove();
  ...
  function makeTextControl(control, control_wrap) {
    ...
    changer.on('change', function (d) {
      ...
      _this11.changeOption(control.option, value);
    });
  }
  ...
  function makeCheckboxControl(control, control_wrap) {
    ...
    changer.on('change', function (d) {
      ...
      _this17.changeOption(d.option, value);
    });
  }
  ...
  function makeBtnGroupControl(control, control_wrap) {
    ...
    changers.on('click', function (d) {
      ...
      _this18.changeOption(control.option, d);
    });
  }
  ...
  function checkRequired$1(dataset) {
    ...
    this.config.inputs.forEach(function (e, i) {
      if (e.type === 'subsetter' && colnames.indexOf(e.value_col) === -1) {
        _this20.config.inputs = _this20.config.inputs.splice(controls[i], 1);
    ...
    this.targets.forEach(function (e) {
      if (option instanceof Array) {
        option.forEach(function (o) {
          return _this21.stringAccessor(e.config, o, value);
        });
      } else {
        _this21.stringAccessor(e.config, option, value);
      }
      e.draw();
    });
  }
